...
import {formatToLocalTime, iconUrlFromCode} from "../services/wetherService";
    ...
    return (
        <div>
            ...
            <div className="flex flex-row items-center justify-between text-white py-3">
                <img src= {iconUrlFromCode(icon)}
                     alt=""
                     className="w-20 "/>
                <p className="text-5xl">{`${temp.toFixed().replace(/.$/, '')}`}&deg;</p>
                ...
                justify-center">
                        ...
                        Real fell:
                        <span className="font-medium ml-1">{`${feels_like.toFixed().replace(/.$/, '')}`}&deg;</span>
                    </div>
                    <div className="flex font-light text-sm items-center
                justify-center">
        ...
        text-white text-sm">
                ...
                <p className="font-light"> High:
                    <span className="font-medium ml-1">{`${temp_max.toFixed().replace(/.$/, '')}`}&deg;</span>
                </p>
                <p className="font-light">|</p>
                <UilSun/>
                <p className="font-light"> Low:
                    <span className="font-medium ml-1">{`${temp_min.toFixed().replace(/.$/, '')}`}&deg;</span>
                </p>
            </div>
        </div>
